...
func writeSummaryMarkdown(outputDir, suiteName string, m SuiteMetrics) error {
	...
	w := bufio.NewWriter(f)
	// bufio.Writer records the first write error and surfaces it on Flush, so
	// the per-line Fprintf returns are intentionally not checked individually.
	...
	if err := w.Flush(); err != nil {
		return fmt.Errorf("flushing summary: %w", err)
	}
